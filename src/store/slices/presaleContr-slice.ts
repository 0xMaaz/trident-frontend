...
interface IGetPresaleContrDetails {
    provider: StaticJsonRpcProvider | JsonRpcProvider;
    networkID: Networks;
    address: string;
}

export interface IPresaleContrDetails {
    contract: string;
    claimablePsi: string;
...
export const getPresaleContrDetails = createAsyncThunk("presaleContr/getPresaleDetails", async ({ provider, networkID, address }: IGetPresaleContrDetails, { dispatch }) => {
    let claimablePsi = "",
        amountBuyable = "",
        claimedPsi = "",
        vestingStart = "",
    ...
    let contrContract = new Contract(addresses.presaleContributor, PresaleContract, provider);
    let term = await contrContract.terms(address);
...
export const changeApproval = createAsyncThunk("presaleContr/changeApproval", async ({ provider, networkID, presaleAddress, address }: IChangeApproval, { dispatch }) => {
    if (!provider) {
...
export const buyPresale = createAsyncThunk("presaleContr/buyPresale", async ({ value, presaleAddress, provider }: IBuyPresale, { dispatch }) => {
...
export const claimPresale = createAsyncThunk("presaleContr/claimPresale", async ({ address, presaleAddress, networkID, provider, stake }: IClaimPresale, { dispatch }) => {
    if (!provider) {
        dispatch(warning({ text: messages.please_connect_wallet }));
        return;
    }
...
export interface IPresaleContrSlice {
    loading: boolean;
...
const initialState: IPresaleContrSlice = {
    loading: true,
    approvedContractAddress: "",
    claimablePsi: 0,
    amountBuyable: "",
...
const presaleContrSlice = createSlice({
    name: "presaleContr",
    initialState,
    reducers: {
    ...
    extraReducers: builder => {
        builder
            .addCase(getPresaleContrDetails.pending, state => {
                state.loading = true;
            })
            .addCase(getPresaleContrDetails.fulfilled, (state, action) => {
                setAll(state, action.payload);
                state.loading = false;
            })
            .addCase(getPresaleContrDetails.rejected, (state, { error }) => {
                state.loading = false;
                console.log(error);
            });
    },
...
export default presaleContrSlice.reducer;

export const { fetchPresaleSuccess } = presaleContrSlice.actions;

const baseInfo = (state: RootState) => state.presaleContr;

export const getPresaleContrState = createSelector(baseInfo, claiming => claiming);